...
declare namespace THING {
  ...
  interface ThingMouseEvent<T extends string = 'click'>
    ...
    pickedObj: BaseObject;
    __lastObject__: BaseObject;
  }
  interface MouseenterEvent extends Event<'mouseenter', BaseObject> {
    /** 目标对象 */
    mouseOnObject: BaseObject;
    stopPropagation: Function;
    tag: undefined;
    x: number;
    y: number;
  }
  interface MouseleaveEvent extends Event<'mouseleave', BaseObject> {
    /** 目标对象 */
    mouseOffObject: BaseObject;
    stopPropagation: Function;
    tag: undefined;
    x: number;
    y: number;
  ...
  interface App extends THING.AppProp {
    ...
    create<R = BaseObject>(
      param: {
        ...
        element?: HTMLElement;
        /** 是否忽略父物体包围盒 */
        ignoreParentBoundingBox?: boolean;
        /** 界面轴心 */
        pivot?: [number, number];
        /** 坐标点 */
        coordinates?: number[] | number[][];
     ...
     * 物体查询
     * @param param 查询条件
     *  @see 字符串部分参考：http://www.w3school.com.cn/jquery/jquery_ref_selectors.asp
     * @example
     * ```
     * // 查询 id 为 001 的对象集合
     * app.query('#001');
     ...
     * ```
     */
    query<T extends BaseObject>(param: string | RegExp): THING.Selector<T>;
...
heatMap.setGradient({ 0.4: 'blue', 0.6: 'cyan', 0.7: 'lime', 0.8: 'yellow', 1.0: 'red' })
  ...
  class Selector<T = BaseObject> {
     ...
     * ```
     */
    add(param: any, index?: number): THING.Selector<T> | void;
     ...
     */
    filter<S extends T>(
      callbackfn: (value: S, index: number, array: S[]) => boolean
    ): THING.Selector;
     ...
     * ```
     */
    not(param: any): THING.Selector<T>;
     ...
     * ```
     */
    query(param: string | RegExp): THING.Selector<T>;
